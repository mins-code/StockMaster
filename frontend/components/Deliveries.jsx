...
const Deliveries = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch documents
        const documentResponse = await axios.get('http://localhost:3000/inventory/DELIVERY', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:3000/inventory/DELIVERY', formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleValidate = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://localhost:3000/inventory/DELIVERY/${id}/validate`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setError(err.response?.data?.error || 'Failed to validate delivery order');
    }
  };
  ...
  return (
    <div className="p-8 space-y-8">
      {/* Heading */}
      <h1 className="text-3xl font-bold text-dark-text">Delivery Orders</h1>
      ...
      {/* Document List */}
      <div className="app-card p-6">
        <h2 className="text-xl font-semibold mb-4">Delivery Orders List</h2>
